...
impl Solution {
    ...
    pub fn longest_consecutive_hashset(&self, nums: Vec<i32>) -> i32 {
        if nums.is_empty() {
            return 0;
        }
        ...
        for &num in &num_set {
            ...
            if !num
                .checked_sub(1)
                .map_or(false, |prev| num_set.contains(&prev))
            {
    ...
    /// Time Complexity: O(n)
    /// Space Complexity: O(n)
    pub fn longest_consecutive_boundary_map(&self, nums: Vec<i32>) -> i32 {
        if nums.is_empty() {
            return 0;
        }
    ...
    pub fn longest_consecutive_sorting(&self, nums: Vec<i32>) -> i32 {
        if nums.is_empty() {
            return 0;
        }
    ...
    /// Time Complexity: O(n)
    /// Space Complexity: O(n)
    pub fn longest_consecutive_range_merge(&self, nums: Vec<i32>) -> i32 {
        if nums.is_empty() {
            return 0;
        }
    ...
    /// Time Complexity: O(n)
    /// Space Complexity: O(n)
    pub fn longest_consecutive_bidirectional(&self, nums: Vec<i32>) -> i32 {
        if nums.is_empty() {
            return 0;
        }
...
mod tests {
    ...
    fn test_consistency_across_approaches() {
        ...
        for nums in test_cases {
            ...
            assert_eq!(
                result1, result2,
                "HashSet vs UnionFind mismatch for {:?}",
                nums
            );
            assert_eq!(
                result2, result3,
                "UnionFind vs BoundaryMap mismatch for {:?}",
                nums
            );
            assert_eq!(
                result3, result4,
                "BoundaryMap vs Sorting mismatch for {:?}",
                nums
            );
            assert_eq!(
                result4, result5,
                "Sorting vs RangeMerge mismatch for {:?}",
                nums
            );
            assert_eq!(
                result5, result6,
                "RangeMerge vs Bidirectional mismatch for {:?}",
                nums
            );
        }
    }
